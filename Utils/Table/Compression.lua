...
local function compress(t)
	local text = HttpService:JSONEncode(t)
	...
	listkey(key)
	spans[width] = span
	return "cmprsd" .. table.concat(spans, ",").."|"..table.concat(sequence)
end
...
local function decompress(text)
	...
	return HttpService:JSONDecode(unescape(table.concat(sequence)):gsub("^cmprsd", ""))
end
